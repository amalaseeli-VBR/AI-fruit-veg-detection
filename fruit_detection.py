...
_last_db_future = None

def _submit_db_task(func, *args):
    global _last_db_future
    with _db_lock:
        if _last_db_future and not _last_db_future.done():
            return
        _last_db_future = _db_executor.submit(func, *args)

def queue_save(payload_dict):
    _submit_db_task(save_detected_product, json.dumps(payload_dict))

def queue_clear():
    _submit_db_task(clear_database)

def _ensure_cv2_gui_support():
    """Provide no-op fallbacks when HighGUI symbols are unavailable (e.g., headless builds)."""
    if not hasattr(cv2, "imshow"):
        def _imshow_stub(*args, **kwargs):
            return None
...
if __name__ == "__main__":
    try:
        ...
        print(f"Warning: could not seed products (skipping): {exc}")
    main()
